...
#else
#define ACRX_AUTO_INIT_QUEUE_EXPR(CLASS_NAME) acrxQueueClassForInitialization(&CLASS_NAME::gpDesc, &CLASS_NAME::rxInit, ACRX_T(#CLASS_NAME))
#endif
...
#ifndef ACRX_ALLOW_GPDESC
#define ADESK_DEPRECATE_GPDESC ADESK_DEPRECATE_FOR_INTERNAL_USE
#else
#define ADESK_DEPRECATE_GPDESC 
#endif

#define ACRX_DECLARE_GPDESC(CLASS_NAME) \
    ADESK_DEPRECATE_GPDESC static AcRxClass* gpDesc; 
...
// Initialze the gpdesc variable based on the class name
#define ACRX_GPDESC_INIT(CLASS_NAME) \
    ADESK_SUPPRESS_DEPRECATED \
    if (gpDesc == nullptr) { \
        ADESK_SUPPRESS_DEPRECATED \
        gpDesc = ::acrxFindAcRxClass(ACRX_T(#CLASS_NAME)); \
    } \
    ACRX_CLASS_INIT_CHECK(CLASS_NAME)
...
#if defined(_DEBUG)
#include "acutasrt.h"
    // Reinitializion of an AcRxClass is usually an error.  rxInit & deleteAcRxClass are not 
    // reference counted, so double initialization can lead to double deletion
    #define ACRX_STATIC_CHECK_ERROR(CLASS_NAME) \
    ADESK_SUPPRESS_DEPRECATED \
    if (gpDesc) \
       acutAssertMessage(ACRX_T(#CLASS_NAME L" already initialized"), ACRX_T(__FILE__), __LINE__, 0);
#else
    #define ACRX_STATIC_CHECK_ERROR(CLASS_NAME)
#endif

// Only enable the initialization check in Fabric for now
#if defined(_DEBUG) && defined(_ADESK_CROSS_PLATFORM_) && !defined(_ADESK_MAC_)
    #define ACRX_CLASS_INIT_CHECK(CLASS_NAME) \
    ADESK_SUPPRESS_DEPRECATED \
    if (!gpDesc) \
        acutAssertMessage(ACRX_T(#CLASS_NAME L" is being used, but the class is not yet initialized!"), ACRX_T(__FILE__), __LINE__, 0);
#else
    #define ACRX_CLASS_INIT_CHECK(CLASS_NAME)
#endif
...
#define ACRX_STATIC_CHECK(CLASS_NAME) \
    if (gpDesc!=nullptr) { \
        ACRX_STATIC_CHECK_ERROR(CLASS_NAME); \
