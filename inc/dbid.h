...
#if __cpp_impl_three_way_comparison
    auto operator<=>(const AcDbObjectId&) const;
    auto operator<=>(const AcDbStub* pStub) const;
#else
...
inline auto AcDbObjectId::operator<=>(const AcDbObjectId& compare) const {
    return mId <=> compare.mId;
}

inline auto AcDbObjectId::operator<=>(const AcDbStub* pStub) const {
    return mId <=> pStub;
}
