...
class AcApProfileManager
{
    public:
        ...
        // Deprecated - forwards to the AcString-based overload
#define AcProf_Deprecated_ACHAR_Methods
#ifdef AcProf_Deprecated_ACHAR_Methods
...
#ifdef AcProf_Deprecated_ACHAR_Methods
inline void AcApProfileManager::ProfileRegistryKey(ACHAR *& strRegProfileKey,
                                                const ACHAR * strProfileName)
{
    const AcString str = this->ProfileRegistryKey(strProfileName);
    strRegProfileKey = new ACHAR[str.length() + 1];
    for (unsigned i = 0; i <= str.length(); i++)
        strRegProfileKey[i] = str[i];
}
#endif
