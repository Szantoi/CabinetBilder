...
public:
    ...
    /// <summary> Check if the MDI frame window is floating </summary>
    ACAD_PORT bool isFloating() const;

    /// <summary>
    /// Returns true if this is a floating frame and the floating frame is pinned.
    /// </summary>
    ACAD_PORT bool isPinned() const;
...
public:
    ...
    ACAD_PORT WindowHandle windowHandle() const ADESK_OVERRIDE;
...
public:
    ...
    /// <summary>Fired when a document window is to be docked from floating dwg frame back to the MDI child frame.</summary>
    /// <param name="docWindow">The document window to be docked. </param>
    ACAD_PORT virtual void documentWindowToBeDocked(AcApDocWindow* docWindow);
};
