...
//
// 1. Regular copying and assignment by reference are prohibited.
//    Derived classes must also prohibit regular copying and
//    assigment.  Move copying and assignment by rvalue reference
//    are allowed where ownership is transferred from the old
//    pointer to the new one.  These classes contain a pointer to
...
public:
    AcDbNamedObjectDictionaryPointer()
        : AcDbDictionaryPointer() {}
    AcDbNamedObjectDictionaryPointer(AcDbObjectId objId, AcDb::OpenMode mode = AcDb::kForRead)
        : AcDbDictionaryPointer(objId, mode) {}
    AcDbNamedObjectDictionaryPointer(AcDbDatabase* pDb, AcDb::OpenMode mode = AcDb::kForRead)
        : AcDbDictionaryPointer()
    {
        this->m_status = (pDb == nullptr) ? Acad::eNullObjectPointer : open(pDb, mode);
    }

    AcDbNamedObjectDictionaryPointer(AcDbNamedObjectDictionaryPointer&&) = default;
    ...
    {
        return (pDb == nullptr) ? Acad::eNullObjectPointer : AcDbDictionaryPointer::open(pDb->namedObjectsDictionaryId(), mode);
    }

private:
    // Copy and assignment prohibited.
    AcDbNamedObjectDictionaryPointer(const AcDbNamedObjectDictionaryPointer&) = delete;
    AcDbNamedObjectDictionaryPointer& operator=(const AcDbNamedObjectDictionaryPointer&) = delete;
...
template<class T_OBJECT> inline AcDbObjectPointerBase<T_OBJECT>&
AcDbObjectPointerBase<T_OBJECT>::operator=(AcDbObjectPointerBase&& other)
{
    close();
