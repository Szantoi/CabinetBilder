...
public:
    ...
    ~AcMgObjectFactory()
    {
        // At this point it's possible the module has already been unloaded.
        // Calling desc() again will just attempt to initialize the RX class
        auto desc = acrxFindAcRxClass(ImpObj::className());
        if (desc)
            desc->delX(AcMgObjectFactoryBase::desc());
    }
