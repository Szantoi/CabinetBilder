...
{
    //bounding box representd as a rectangle ABCD
    //bottom left - bBox[0] = A
    //top left - bBox[1] = B
    //bottom right - bBox[3] = D
    //M - pick point
    ...
    // if AM * AD < 0 can be interpreted as M being outside the rectangle
    // if AM * AD > AD * AD then M would also be outside since the projection of AM would be longer than AD.
    // same idea for AB, AM
    //is pt M inside the text's bounding box rectangle?
...
#define AcDbText_Deprecated_ACHAR_Methods
#ifdef AcDbText_Deprecated_ACHAR_Methods
    ...
    //pick in space check for Text when TEXTGAPSELECTION is enabled
    ACDBCORE2D_PORT bool hitTest(const AcGePoint3d& ptHit) const;
    //get shrink wrapped bounding box for Text
...
#define AcDbAttr_Deprecated_ACHAR_Methods
#ifdef AcDbAttr_Deprecated_ACHAR_Methods
    ...
    Acad::ErrorStatus   adjustAlignment(
...
#ifdef AcDbAttr_Deprecated_ACHAR_Methods
// These overloads are deprecated. They allocate a char buffer that the caller is
// responsible for freeing. Please use the other overloads, taking an (AcString &) arg
//
...
#define AcDbShape_Deprecated_ACHAR_Methods
#ifdef AcDbShape_Deprecated_ACHAR_Methods
...
#define AcDbVport_Deprecated_ACHAR_Methods
#ifdef AcDbVport_Deprecated_ACHAR_Methods
    ...
    void      subObjModified(const AcDbObject* pDbObj, 
