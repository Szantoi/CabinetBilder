...
#include "ads.h"
#include "acutmem.h"    // acutNewString()
...
#define AcEd_Deprecated_ACHAR_Funcs
#ifdef AcEd_Deprecated_ACHAR_Funcs
inline Acad::ErrorStatus acedGetCommandForDocument(const AcApDocument* pDoc, // deprecated
...
ACAD_PORT bool acedSetColorPrompt( const ACHAR* promptString,
                                   AcCmColor& color,
 ...
 * should first save the return value from acedIsUpdateDisplayPaused(), 
 * then restore that value when finished pausing the display. 
 * NOTE: Direct use of this call is no longer recommended, callers should
 *       use AcEdUpdateDisplayPauseScope class instead to ensure
 *       matching save/restore of the prior state.
 */
...
public:
    ...
    AcEditorReactor() {}
    ACCORE_PORT virtual ~AcEditorReactor();
    ...
    // Insert Events.
    //
    void beginInsert(AcDbDatabase* /*pTo*/, const ACHAR* /*pBlockName*/,
                             AcDbDatabase* /*pFrom*/) override {}
    void beginInsert(AcDbDatabase* /*pTo*/, const AcGeMatrix3d& /*xform*/,
                             AcDbDatabase* /*pFrom*/) override {}
    void otherInsert(AcDbDatabase* /*pTo*/, AcDbIdMapping& /*idMap*/,
                             AcDbDatabase* /*pFrom*/) override {}
    ...
    /// <summary>
    /// Fired when one or more commands will be redone.
    /// </summary>
    /// <param name="activity">typically will be 0.</param>
    /// <param name="num">number of commands being redone.</param>
    virtual void redoSubcommandNumber(int /*activity*/, int /*num*/) {}
    
    /// <summary>
    /// Fired when a subcommand boundary is written to the undo file.
    /// </summary>
    /// <param name="boundaryType">see the undoBoundary enum in dbundo.h</param>
    virtual void undoWriteBoundary(int /*boundaryType*/) {}
    
    /// <summary>
    /// Fired when one or more subcommands are about to be undone.
    /// </summary>
    /// <param name="num">number of commands being undone.</param>
    virtual void subcommandsWillBeUndone(int /*num*/) {}
    ...
    virtual Acad::ErrorStatus xrefSubCommandStart(
            ...
            const ACHAR* const * /*paths*/) { return Acad::eOk; }
...
// AutoCAD Color Index to RGB conversion function
// Takes an AutoCAD Color Index
// Returns an RGB color value in Win32 COLORREF (0x00bbggrr) format
ACAD_PORT Adesk::ColorRef acedGetRGB(int color);
...
enum BlockEditModeFlags {
    ...
    kBlkEditModeDirty  = 0x04,      // Block has been modified since last BSave or Save
    kBlkEditModeBSaved = 0x08       // A BSave has been done since last dwg Save
};
...
/// <summary>
/// This function is used to show or hide the blue border.
/// </summary>
/// <param name="bShow">
/// Set this to true to show blue border, set to false to hide blue border.
/// </param>
/// <returns>
...
/// </summary>
ACCORE_PORT bool acedGetFullSubentPathArray(AcEdSelectionSetService* service, bool bIsSubSelectionIndex, int index, AcDbFullSubentPathArray& fullPaths);

/// <summary>
/// Gets the number of ignored entities and duplicated blocks from selection set.
/// </summary>
ACCORE_PORT void acedSetIgnoredEntAndDuplicatedBlkCount(AcEdSelectionSetService* service, int ignoredEntCount, int duplicatedBlkCount);
...
/// <summary>
/// wrapper class to correctly save/set/restore the state of acedUpdateDisplayPause within a scope
/// can also use default constructor to allow manual set/restore for callback type of patterns
/// AutoCAD code only uses this class, it does not directly use acedUpdateDisplayPause
/// This is not a stack, so only a single save state is maintained.
/// </summary>
class AcEdUpdateDisplayPauseScope
{
  public:
    ...
    /// </summary>
    AcEdUpdateDisplayPauseScope() {}            // client will use set and restore methods - one time use only.

    /// <summary>
    /// Constructor, taking in a parameter to be passed to SaveAndSet
    /// </summary>
    /// <param name="bEnable">
    /// bool. True to disable screen updates, false to enable screen update
    /// </param>
    AcEdUpdateDisplayPauseScope( bool bEnable ) // client using scope set/restore
    {
        SaveAndSet( bEnable );
    }

    /// <summary>
    /// Default destructor. Uses the Restore method to ensure any saved state is restored.
    /// </summary>
    ...
    /// <summary>
    /// Save the existing state and sets the new disable state. Can only be used once, additional calls are ignored.
    /// </summary>
    /// <param name="bEnable">
    /// bool. True to disable screen updates, false to enable screen update
    /// </param>
    void SaveAndSet( bool bDisable )    // one time use only, save old value and set new value
    {
        if (!m_bDidSet)     // this is not a stack, one time use only!
        {
            m_bDidSet = true;
            m_bOldValue = acedIsUpdateDisplayPaused();  // save current value
            acedUpdateDisplayPause( bDisable );         // set new disable state
        }
    }

    /// <summary>
    /// Restore any previously saved disable state. Subsequent calls are ignored.
    /// Once called, it is now valid to call SaveAndSet again.
    /// </summary>
    void Restore()              // one time use only, restore prior saved value
    {
        if (m_bDidSet)      // only restore if we have not done it yet
        {
            m_bDidSet = false;                      // restoring value.
            acedUpdateDisplayPause( m_bOldValue );  // set new disable state
        }
    }
