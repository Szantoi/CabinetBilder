...
{
protected:
// TODO: remove this when binary compability requirements are removed
#if !defined(_WIN64) && !defined (_AC64)
    using ACPL_ULONG_PTR = unsigned long;
#else
...
public:
    ...
    //Severe handler
    ACPL_PORT virtual void severeError(ACPL_ULONG_PTR category,
                                       const unsigned int specific,
                                       const ACHAR*       pLocation,
                                       const ACHAR*       pContextData,
                                       const ACHAR*       pRevision);
    ...
    //Error handler
    ACPL_PORT virtual ErrorResult error(ACPL_ULONG_PTR category,
                                        const unsigned int specific,
                                        const ACHAR*       pLocation,
                                        const ACHAR*       pContextData,
                                        const ACHAR*       pRevision);
    ...
    ACPL_PORT virtual ErrorResult ariError(ACPL_ULONG_PTR category,
                                           const unsigned int specific,
                                           const ACHAR*       pLocation,
                                           const ACHAR*       pContextData,
                                           const ACHAR*       pRevision);
    ...
    ACPL_PORT virtual void terminalError(ACPL_ULONG_PTR category,
                                         const unsigned int specific,
                                         const ACHAR*       pLocation,
                                         const ACHAR*       pContextData,
                                         const ACHAR*       pRevision);
...
protected:
    ...
    //info error handler.
    ACPL_PORT void appInfo(ACPL_ULONG_PTR category,
                           const unsigned int specific,
                           const ACHAR*       pLocation,
                           const ACHAR*       pContextData,
                           const ACHAR*       pRevision);
    ...
    //warning error handler.
    ACPL_PORT ErrorResult appWarning(ACPL_ULONG_PTR category,
                                     const unsigned int specific,
                                     const ACHAR*       pLocation,
                                     const ACHAR*       pContextData,
                                     const ACHAR*       pRevision);
    ...
    //Severe handler
    ACPL_PORT void appSevereError(ACPL_ULONG_PTR category,
                                  const unsigned int specific,
                                  const ACHAR*       pLocation,
                                  const ACHAR*       pContextData,
                                  const ACHAR*       pRevision);
    ...
    //Error handler
    ACPL_PORT ErrorResult appError(ACPL_ULONG_PTR category,
                                   const unsigned int specific,
                                   const ACHAR*       pLocation,
                                   const ACHAR*       pContextData,
                                   const ACHAR*       pRevision);
    ...
    //Abort/Retry/Ignore handler
    ACPL_PORT ErrorResult appARIError(ACPL_ULONG_PTR category,
                                      const unsigned int specific,
                                      const ACHAR*       pLocation,
                                      const ACHAR*       pContextData,
                                      const ACHAR*       pRevision);
    ...
    // Terminal error handler
    ACPL_PORT void appTerminalError(ACPL_ULONG_PTR category,
                                    const unsigned int specific,
                                    const ACHAR*       pLocation,
                                    const ACHAR*       pContextData,
                                    const ACHAR*       pRevision);
