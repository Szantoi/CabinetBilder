...
// Disable warning about unnamed union to allow building
// with warning level 4 and treat warnings as errors.
//
#pragma warning(push)
...
inline bool AcDbHandle::restZeros(int i) const
{
    if (i < 0 || i >= kMaxValueBytes)
        return false;
    if (i < 4) {
        const Adesk::UInt32 mask = ~0U << (i << 3);
        return !(mHandle.mHigh | (mHandle.mLow & mask));
    } else {
        const Adesk::UInt32 mask = ~0U << ((i - 4) << 3);
