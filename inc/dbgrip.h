...
public:
    AcDbGripData();
    /// <summary> This constructor enables setting of pt and appData members.  All other members are set to NULL </summary>
    /// <param name="AcGePoint3d"> Set the m_gripPt data member </param>
    /// <param name="AppData"> Set the m_pAppData data member </param>
    AcDbGripData(const AcGePoint3d&, void* AppData);
    ADESK_DEPRECATE_FOR_INTERNAL_USE AcDbGripData(const AcGePoint3d&, void*, GripOperationPtr hotGrip,
    ...
    ADESK_DEPRECATE_FOR_INTERNAL_USE AcDbGripData(const AcGePoint3d&, void*, AcRxClass*,
...
private:
    ...
    AcGePoint3d*        m_pAltBasePt = nullptr;
    void*               m_pAppData = nullptr;
    GripOperationPtr    m_pHotGripFunc = nullptr;
    GripOperationPtr    m_pHoverFunc = nullptr;
    union {
        GripRtClkHandler oldHandler;
        GripRightClickHandler newHandler;
    } m_pRtClk;
    GripWorldDrawPtr    m_pWorldDraw = nullptr;
    GripViewportDrawPtr m_pViewportDraw = nullptr;
    GripOpStatusPtr     m_pGripOpStatFunc = nullptr;
    GripToolTipPtr      m_pToolTipFunc = nullptr;
    GripDimensionPtr    m_pHoverDimensionFunc = nullptr;
    GripDimensionPtr    m_pHotGripDimensionFunc = nullptr;
    unsigned int        m_bitFlags = 0;
    GripInputPointPtr   m_pInputPointFunc = nullptr;
    AcRxClass*          m_pAppDataClass = nullptr;

    GripCLIPromptPtr          m_pCLIPromptFunc = nullptr;
    GripCLIDisplayStringPtr   m_pCLIDisplayStringFunc = nullptr;
};
...
inline AcDbGripData::AcDbGripData(const AcGePoint3d& pt, void* AppData,
    ...
    m_gripPt(pt), m_pAltBasePt(altBasePt), m_pAppData(AppData), m_pHotGripFunc(hotGrip),
...
inline AcDbGripData::AcDbGripData(const AcGePoint3d& pt, void* AppData,
    ...
    unsigned int bitFlags, AcGePoint3d* altBasePt,
    GripInputPointPtr inputPointFunc) :
    m_gripPt(pt), m_pAltBasePt(altBasePt), m_pAppData(AppData),
    m_pHotGripFunc(hotGrip), m_pHoverFunc(hoverGrip),
    m_pWorldDraw(wd), m_pViewportDraw(vd), m_pGripOpStatFunc(stat),
    m_pToolTipFunc(tt), m_pHoverDimensionFunc(hoverDim),
    m_pHotGripDimensionFunc(hotGripDim),
    m_bitFlags(bitFlags),
    m_pInputPointFunc(inputPointFunc), m_pAppDataClass(pAppDataClass)
{
    setRtClk(RtClk);
}
