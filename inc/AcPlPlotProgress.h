...
#if defined(_Ac_String_h_) && defined (_ACUTMEM_H)
#define AcPlPlotProg_Deprecated_AMAKE_Methods
#endif
...
public:
    ...
    virtual bool getStatusMsgString(AcString & sMsg) const = 0;
#ifdef AcPlPlotProg_Deprecated_AMAKE_Methods
...
ACPL_PORT AcPlPlotProgressDialog*
...
#ifdef AcPlPlotProg_Deprecated_AMAKE_Methods
inline bool AcPlPlotProgress::getStatusMsgString(ACHAR *& pMsg) const
{
    AcString sOut;
    const bool bRet = this->getStatusMsgString(sOut);
    // Note previous string ptr is not freed and is only updated on success
    if (bRet)
        ::acutNewString(sOut.constPtr(), pMsg);
    return bRet;
}

inline bool AcPlPlotProgressDialog::getPlotMsgString(PlotMSGIndex index, ACHAR *& pMsg) const
{
    AcString sOut;
    const bool bRet = this->getPlotMsgString(index, sOut);
    // Note previous string ptr is not freed and is only updated on success
    if (bRet)
        ::acutNewString(sOut.constPtr(), pMsg);
    return bRet;
