...
// DESCRIPTION:
//
// This file contains the definition for a dynamic array, called AcArray<T,R>, of
// objects of type "T".
// It's similar to the standard container class std::vector<>
//
// "Dynamic" means that the array can grow at runtime, unlike the old "C style" array.
//
...
//            - The minimum number of elements the array grows by.
//
...
// length, or double the current physical length if the space used
// is less than 1M bytes, or the number items that will grow
// the total space by an additional 1M bytes.
//
// The R class is one of two available "allocators", used to select either
// "memory copying" (which is faster) or "object copying" behavior.
// The correct allocator is normally select automatically for a given
// array item type T, using type detection.
//
// A larger grow length may improve performance if many small append operations
// are going to be done on the array.  It's also a good idea to pre-allocate the
// array up front in that case, using setLogicalLength() or setPhysicalLength().
//
// Note on size values: this class uses a 32-bit signed int for size and index values.
// Therefore the highest physical and logical length values are 2G-1
//
...
// Use whatever assert macro the client defines, if any
#ifdef ASSERT
...
public:
    // ctor is explicit to disallow uses like:  AcArray<int> arr = 1;
    //
    ...
    // nMaxLength is only used here in asserts, so does not actually restrict the array size
    // The old maxLength() method returned nMaxInt / sizeof(T), but now it's simply nMaxInt
    //
    enum {      nMaxLength = Adesk::nMaxInt,    // the most elements this array can contain
                nInvalidIndex = -1,             // returned by find() when element not found
                nGeometricGrowthThreshold = Adesk::n1Meg};
    ...
    // Does not take a value arg, so new element takes default value, if any
    ...
    // If this array is empty, then just take over the other array's buffer and size
    // info, leaving the other array empty.
    // Otherwise, move or copy the other array's elements to the end of this array, leaving
    // the other array with its original length but with its element possibly "empty"
    //
    ...
    template<typename... TV> AcArray<T,R> & appendList (const TV & ... args)
    {
    ...
    // Array length.
    // Note that logical and physical length values are in items, not bytes
    //
    int                 length      () const;           // Same as logical length.
    bool                isEmpty     () const;
    int                 logicalLength() const;          // number of existing items
    AcArray<T,R>&       setLogicalLength(int);
    AcArray<T,R>&       setLogicalLength(int, const T& value);  // init new cells to value
    int                 physicalLength() const;         // current capacity in items
    AcArray<T,R>&       setPhysicalLength(int);

    // Automatic resizing.
    // The minimum number of items to increase the buffer by, when it needs to grow
    // This value is only used if it's larger than the current buffer size
    // Otherwise we double the current buffer size (or add 1M to it)
    //
...
private:
    ...
    AcArray<T,R> & assignHelper (int)
    {
        // Gets called if no args are passed to appendList()
        return *this;
    }
    AcArray<T,R> & assignHelper (int nIndex, const T & value)
    {
        this->setAt(nIndex, value);
        return *this;
    }
    template<typename... TV> AcArray<T,R> & assignHelper (int nIndex, const T & value,
                                                          const TV & ... args)
    {
...
{ return this->findFrom(value, start) != nInvalidIndex; }
...
AcArray<T,R>::append()
{
    const int nIdx = this->logicalLength();
    this->setLogicalLength(nIdx + 1);
    return this->at(nIdx);
}
...
//
template <class T, class R> inline
AcArray<T,R>::AcArray(const AcArray<T,R>& src) : mGrowLen(src.mGrowLen)
{
...
// Move ctor
template <class T, class R> inline
AcArray<T,R>::AcArray(AcArray<T,R>&& src) : mGrowLen(src.mGrowLen)
{
...
AcArray<T,R>::setLogicalLength(int n)
{
    ...
    AC_ARRAY_ASSERT(n <= nMaxLength);
    if (n > mPhysicalLen) {

        // See comment at top about the growth algorithm.  It varies depending on
        // current array size and the growLen.
        // Initially we grow small arrays by a small fixed amount (mGrowLen elements)
        // Then we grow medium sized arrays by doubling the buffer size.
        // Finally, larger arrays are grown by a larger fixed amount (1M bytes)
        //
        const int growth = (mPhysicalLen * sizeof(T)) < nGeometricGrowthThreshold ?
                        mPhysicalLen : (nGeometricGrowthThreshold / sizeof(T));

        int minSize = mPhysicalLen + std::max<int>(growth, mGrowLen);

        // if client is requesting more than we would grow by, then we have to use
        // the client's requested size
        if ( n > minSize)
            minSize = n;
...
template <class T, class R> AcArray<T,R>&
AcArray<T,R>::setLogicalLength(int n, const T& value)
{
    const int nOldLen = this->mLogicalLen;
                                              ...
                                              const T * pSource, int nCount)
{
    ...
    AC_ARRAY_ASSERT(nCount <= AcArray<T>::nMaxLength);
    if (bSameBuffer) {
...
{
  public:
    // Helper method for arrays of pointers.
    // Deletes all pointers in the array and sets the array to empty.
    // E.g., if arr is an AcArray<SomeType *>, where SomeType is not void,
    //       then we can say:  AcArrayUtil::deletePtrs(arr);
    // if bFreeBuffer is true, then remove AcArray's allocated buffer
    //          Otherwise mem buffer is retained and re-used
    // if bUseArrayDelete is true, then call delete[] instead of delete
    //          For use with ptrs to C-style arrays, as in arr.append(new T[3]);
    //
    template<typename T> static
            AcArray<T *> & deletePtrs(AcArray<T *> &arr, bool bFreeBuffer = false,
    ...
    // Helper method for arrays of void ptrs, which need to be deleted as pointers to
    // a specific type.  Caller specifies the pointer type.
    // E.g.:  AcArray::deleteVoidPtrs<MyType>(arr);  // where arr is AcArray<void *>
    // E.g., if arr is an AcArray<void *>, where the pointers actually are to SomeType,
    //       then we can say:  AcArrayUtil::deleteVoidPtrs<SomeType>(arr);
    // (Note: void ptrs are not type safe and should only be used as a last resort.)
    //
    template<typename T> static
            AcArray<void *> & deleteVoidPtrs(AcArray<void *> &arr, bool bFreeBuffer = false)
    {
        for (void * p : arr) {
...
// Specialization which disallows deletePtrs() calls on VoidPtr arrays.
// VoidPtr arrays are not typesafe and should only be used as a last resort.
//
// Note however that if you really want to delete a VoidPtr array's items
// without specifying a type, you can say deleteVoidPtrs<void>(pVoidPtrArr);
//
template<> AcArray<void *> & AcArrayUtil::deletePtrs<void>(AcArray<void *> &arr,
                                bool bFreeBuffer, bool bUseArrayDelete) = delete;

// Specializations for classes known to be able to use MemCopyReallocator
//
