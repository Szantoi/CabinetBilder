...
/// snapping to, hovering, or heating a grip. Only called if the 
/// AcDbSelectGrips protocol extension is not implemented or is implemented
/// and does not reject selection.
/// </remarks>
///
...
/// <remarks>
/// Currently only called from the AutoCAD grip mechanism before
/// snapping to, hovering, or heating a grip. Called before any 
/// implementations of AcDbSelectGrip. If either implementation rejects
/// selection of the grip then it is rejected. 
/// </remarks>
///

class ADESK_NO_VTABLE AcDbSelectGrip2 : public AcRxObject {
public:
    ACRX_DECLARE_MEMBERS_EXPIMP(AcDbSelectGrip2, ACDBCORE2D_PORT);
    /// <summary>
    /// Determine whether a grip on a custom entity is selectable. 
    /// </summary>
    ///
    ...
    /// </param>
    /// 
    /// <param name="pGripData">
    /// [Input] The AcDbGripData appdata assigned to the custom grip.
	///   Use this along with pVd to determine whether the grip 
	///   is selectable in the current (viewport) context.
    /// </param>
    /// 
    /// <returns>
    ...
    virtual bool SelectGrip(const AcDbEntity * pEnt, 
		                    const AcGiViewportDraw* pVd,
							const AcDbGripData* pGripData) = 0;
};
#pragma pack (pop)
