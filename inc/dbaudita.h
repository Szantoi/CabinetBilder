...
#define ACDB_AUDIT_RETURN(pAuditInfo, baseEs, localEs, boilDownLocalStatus)        \
    return (baseEs != Acad::eOk)          ?  \
               (baseEs)                   :  \
               (Acad::eOk == (localEs))   ?  \
                   Acad::eOk              :  \
                   boilDownLocalStatus    ?  \
                       (pAuditInfo->fixErrors()?  \
                           Acad::eFixedAllErrors :  \
...
public:
    ...
    AcDbAuditInfo();
    ACDBCORE2D_PORT AcDbAuditInfo(AcDbDatabase *pDb);
    ~AcDbAuditInfo();

    ACDBCORE2D_PORT AcDbDatabase *database() const;
    ACDBCORE2D_PORT bool setDatabase(AcDbDatabase * pDb);   // returns true on success
...
class AcDbRecover{
    ...
    public:
    virtual  Acad::ErrorStatus callBack(AcDbDatabase*) = 0;
};
