...
#define AcDb_Field_Deprecated_ACHAR_Methods
#ifdef AcDb_Field_Deprecated_ACHAR_Methods
...
{
    // if the caller wants a return string, then use the AcString version to get the error message
    // and provide an allocated string to the caller
    AcString sErrMsg;       // where to save return message
    ...
    if (pszErrMsg != nullptr) {
        if (sErrMsg.isEmpty())    // no message to return
            *pszErrMsg = nullptr; // provide nullptr
        else
            ::acutNewString(sErrMsg.kwszPtr(), *pszErrMsg); // return copy of string
    }
    return es;
}
...
/// </summary>
///<returns>
/// Returns the global field evaluation option value.
///</returns>
...
/// Input with the new global field evaluation option value.
/// </param>    
///<returns>
/// Returns Acad::eOk if successful. Otherwise, returns an AutoCAD error status.
///</returns>
ACDBCORE2D_PORT Acad::ErrorStatus acdbSetGlobalFieldEvaluationOption(AcDbField::EvalOption nEvalOption);
